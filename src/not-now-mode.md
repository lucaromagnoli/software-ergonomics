...
- **Can users abuse it?** Less than the question suggests — by construction. Everything the mode suppresses is, by definition, material whose deferral does not harm the user's task; someone who lives in it permanently forgoes tips, tours, and offers, and nothing else. The worst a user can do with this control is dent engagement metrics, which says more about the suppressed material than about the user. One genuine cost is worth naming: some deferred housekeeping — account-recovery setup, backup prompts — is deferrable today but costly never. The deferred queue needs an eventual path back into view.
- **The signal must not become a target.** An app that can ask *is this user in Not Now Mode?* learns something sensitive: this person may be stressed, disrupted, or vulnerable. The same bit that asks software to be quiet could be abused as a targeting input — offers timed to crisis, pricing tuned to the stranded. Mitigations exist: the platform could enforce suppression on the app's behalf rather than exposing a queryable flag, and policy could forbid any adaptive behavior beyond getting quieter. But this risk belongs in the design conversation from the start, not as a patch after the first scandal.
